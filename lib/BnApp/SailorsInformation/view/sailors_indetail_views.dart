...
class TotalSailorDataSearch extends StatefulWidget {
  TotalSailorDataSearch({Key? key, required this.course});
  String? course;
  //String? courseTitle;

  @override
  State<TotalSailorDataSearch> createState() => _DataSearchState();
}

class _DataSearchState extends State<TotalSailorDataSearch> {
  final TotalSailorController controller = Get.put(TotalSailorController());
  ...
  Widget build(BuildContext context) {
    return PopScope(
      onPopInvoked: (_) => controller.forSailorsSearch.clear(),
      child: Obx(
          ...
          // Assuming two schools, adjust as needed
              ...
              appBar: AppBar(
                leading: IconButton(
                  onPressed: (){
                    controller.forSailorsSearch.clear();
                    Get.back();
                  },
                  icon: Icon(Icons.arrow_back),
                ),
            title: Text('Sailorss List',
                style: TextStyle(
                    fontSize: 25,
            ...
            children: [
              ...
              Padding(
                ...
                child: Container(
                  ...
                  child: CupertinoSearchTextField(
                    onChanged: (value) {
                      setState(() {
                        controller.searchSailors(value);
              ...
              Expanded(
                child: DataTableWidget(
                  data: controller.totalsailorsList.where(
...
class DataTableWidget extends StatelessWidget {
  final List<SailorsModel> data;
  final TotalSailorController controller;
...
class LocalCourseGridSource extends DataGridSource {
  ...
  late List<SailorsModel> LocalCourseDataList;
  ...
  List<DataGridRow> get rows  {
    List<SailorsModel> listToUse = controller.forSailorsSearch.isNotEmpty ? controller.forSailorsSearch : LocalCourseDataList;

    return listToUse.asMap().entries.map<DataGridRow>((entry)  {
          int index = entry.key;
          SailorsModel dataRows = entry.value;
