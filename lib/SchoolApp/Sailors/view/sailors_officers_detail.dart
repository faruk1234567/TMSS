...
import '../controller/sailors_controller.dart';
...
class SailorOfficersDataSearch extends StatefulWidget {
  SailorOfficersDataSearch({Key? key, required this.course});
  String? course;
  //String? courseTitle;

  @override
  State<SailorOfficersDataSearch> createState() => _DataSearchState();
}

class _DataSearchState extends State<SailorOfficersDataSearch> {
  final SailorOfficersTraineeController controller = Get.put(SailorOfficersTraineeController());
  ...
  Widget build(BuildContext context) {
    return  PopScope(
      onPopInvoked:(_)=> controller.sailorsSearchList.clear(),
      child: Scaffold(
        ...
        appBar: AppBar(
          leading: IconButton(
            onPressed: (){
              controller.sailorsSearchList.clear();
              Get.back();
            },
            icon: Icon(Icons.arrow_back),
          ),
          title: Text('Sailors List',
              style: TextStyle(
                  fontSize: 24,
          ...
          children: [
            ...
            Padding(
              ...
              child: Container(
                ...
                child: CupertinoSearchTextField(
                  onChanged: (value) {
                    setState(() {
                      controller.forSailorsSEarch(value);
            ...
            Expanded(
              child: DataTableWidget(
                data: controller.SalorsOfficersDataList.where(
...
class DataTableWidget extends StatelessWidget {
  final List<SailorsOfficersModel> data;
  final SailorOfficersTraineeController controller;
...
class LocalCourseGridSource extends DataGridSource {
  ...
  late List<SailorsOfficersModel> LocalCourseDataList;
  ...
  List<DataGridRow> get rows  {
    List<SailorsOfficersModel> LisitOfUser=controller.sailorsSearchList.isNotEmpty? controller.sailorsSearchList:LocalCourseDataList;
    return
      LisitOfUser.asMap().entries.map<DataGridRow>((entry) {
          int index = entry.key;
          SailorsOfficersModel dataRows = entry.value;
