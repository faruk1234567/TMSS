...
import '../controller/reading_material_controller.dart';
...
class SchoolMaterialViewPage extends StatelessWidget {
  SchoolMaterialViewPage({super.key});

  final SchoolReadingMaterialController controller = Get.put(SchoolReadingMaterialController());
        ...
        body: SfDataGridTheme(
          ...
          child: Obx(
                ...
                children: [
                  Expanded(
                    child: SfDataGrid(
                        source: LocalCourseGridSource(controller.SRMateDetails),
                        columns: getColumns(),
                      ...
                      OutlinedButton(
                        onPressed: () async {
                          if (controller.pageNumber.value != 1) {
                            ...
                            await controller.getSchoolReadingMateData();
                          }
                        },
                        child: Text("Previous"),
                      ),
  ...
  return <GridColumn>[
    ...
    GridColumn(
      ...
      label: Container(
        ...
        alignment: Alignment.centerLeft,
        child: const Text('Material Title',
            style: TextStyle(
              fontWeight: FontWeight.bold,
              color: Colors.black,
              fontSize: 25,
    ...
    GridColumn(
      ...
      label: Container(
        padding: const EdgeInsets.all(8),
        alignment: Alignment.center,
        child: const Text('Note', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 25)),
      ),
    ),
...
class LocalCourseGridSource extends DataGridSource {
  ...
  late List<Item> LocalCourseDataList;
  ...
  void buildDataGridRow() {
    dataGridRows = LocalCourseDataList.asMap().entries.map<DataGridRow>((entry) {
      int index= entry.key;
      Item datarows=entry.value;
      return DataGridRow(cells: [
        DataGridCell<String>(columnName: 'Index', value: "${index + 1}"),
        DataGridCell<String>(columnName: 'Instructor', value: '${datarows.readingMaterialTitle} '),
        DataGridCell<String>(columnName: 'shortCode', value: '${datarows.courseName}  '),
