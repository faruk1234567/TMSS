...
class BnaSubListRepo {
  Future<List<BnaSubjectListModel>> getBnaSubDetailList({int? cDurationId}) async {
    ...
    try {
      var response = await http.get(
        Uri.parse(
          // 'http://114.134.95.235:9999/api/sms/dashboard/get-runningCourseDurationfromprocedure?courseTypeId=3&CurrentDate=$date&CurrentDate=$date&viewStatus=1'),
            'http://123.253.147.242:7775/api/sms/bna-subject-name/get-bnaSubjectNameListForJCOsByCourseNameId?courseDurationId=${cDurationId}'),
        headers: {
      ...
      if (response.statusCode == 200) {
        List<BnaSubjectListModel> local = [];
        List data = jsonDecode(response.body);
        for (var i in data) {
          local.add(BnaSubjectListModel.fromJson(i));
        }
        return local;
      } else {
        print(response.statusCode);
        throw "BnaSubList List not Upload ";
      }
    } catch (e) {
      print(e);
      rethrow;
