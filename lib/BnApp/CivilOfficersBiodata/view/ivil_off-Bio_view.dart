...
import '../contrller/Civil_bio_data_controller.dart';
...
class CivilOfficerBioDataView extends StatefulWidget {
  CivilOfficerBioDataView({super.key});

  @override
  State<CivilOfficerBioDataView> createState() => _CivilOfficerBioDataViewState();
}

class _CivilOfficerBioDataViewState extends State<CivilOfficerBioDataView> {
  final CivilOffBioController controller = Get.put(CivilOffBioController());

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text("Civil Instructor BIO Data",style: TextStyle(
            fontSize: 24,
      ...
      body: SfDataGridTheme(
        ...
        child: Obx(
            ...
            children: [
              ...
              Expanded(
                child: SfDataGrid(
                    source: LocalCourseGridSource(controller.CivilOBD,controller),
                    columns: getColumns(),
                  ...
                  OutlinedButton(
                    onPressed: () async {
                      if (controller.pageNumber.value != 1) {
                        ...
                        await controller.getCBdataDetail();
                      }
                    },
                    child: Text("Previous"),
                  ),
  ...
  return <GridColumn>[
    ...
    GridColumn(
      ...
      label: Container(
        padding: const EdgeInsets.all(8),
        alignment: Alignment.center,
        child: const Text('Type', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black, fontSize: 25)),
      ),
    ),
...
class LocalCourseGridSource extends DataGridSource {
  ...
  final CivilOffBioController controller;
  ...
  List<DataGridRow> get rows {
    ...
    return listToUse.asMap().entries.map<DataGridRow>((entry) {
      ...
      return DataGridRow(cells: [
        ...
        DataGridCell<String>(columnName: 'shortCode', value: '${datarows.name} (P NO ${datarows.pno}) '),
        DataGridCell<String>(columnName: 'subjectname', value: '${datarows.localNo.toString()=="null"?"Civil Instructor ":datarows.localNo}' ),
