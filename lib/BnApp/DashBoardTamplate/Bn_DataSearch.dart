...
class DataSearch extends StatefulWidget {
  DataSearch({Key? key, required this.schoolName});
  String? schoolName;
...
class _DataSearchState extends State<DataSearch> {
  ...
  Widget build(BuildContext context) {
    return PopScope(
      onPopInvoked: (_) => controller.forLoccalSearch.clear(),
      child: Obx(
        () => controller.isLoading.value
          ...
          appBar: AppBar(
            leading: IconButton(
              onPressed: (){
                controller.forLoccalSearch.clear();
                Get.back();
              },
              icon: Icon(Icons.arrow_back),
            ),
                  title: Text('Local course',
                      style: TextStyle(
                          fontSize: 25,
                  ...
                  children: [
                    ...
                    Expanded(
                      child: DataTableWidget(
                        data: controller.localCourseList
...
class DataTableWidget extends StatelessWidget {
  final List<LocalModel> data;
 final  LocalCourseController2 controller;
...
class LocalCourseGridSource extends DataGridSource {
  ...
  late List<LocalModel> LocalCourseDataList;
  ...
  List<DataGridRow> get rows{
    List<LocalModel> listToUse = controller.forLoccalSearch.isNotEmpty ? controller.forLoccalSearch : LocalCourseDataList;

    return listToUse.asMap().entries.map<DataGridRow>((entry) {
      int index = entry.key;
      LocalModel dataRows = entry.value;
      ...
      Widget weekText() {
        return InkWell(
            ...
            child: Text(
              '${dataRows.completeWeek}/ ${dataRows.countWeek}  (week)',
              style: TextStyle(
                  fontSize: 25,
                  fontWeight: FontWeight.bold,
                  color: Colors.indigo),
