...
class CourseSubjectListRepo {
  Future<List<CoursesSubjectList>> getCourseSubjectList({int? baseschoolnameid,int? courseNameId,} ) async {
   ...
   /*     http://114.134.95.235:9999/api/sms/course-duration/get-courseDurationByCourseTypeFilter?viewStatus=$viewStatus&courseTypeId=$courseTypeId
      ...
      if (response.statusCode == 200) {
        ...
        List<CoursesSubjectList> SubjectListByCourse = [];
        List data = jsonDecode(response.body);
        for (var i in data) {
          SubjectListByCourse.add(CoursesSubjectList.fromJson(i));
        }
